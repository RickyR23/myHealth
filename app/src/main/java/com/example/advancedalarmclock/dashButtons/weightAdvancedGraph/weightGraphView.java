...
public class weightGraphView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (weightData == null) {
            ...
            int desiredWidth = weightData.size() * 100; // Adjust the width based on the number of data points
    ...
    public void setWeightData(List<String> dateData, List<Integer> weightData, int minWeightValue, int maxWeightValue, int lineColor) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (dateData == null || dateData.isEmpty() || weightData == null || weightData.isEmpty())
            return;
        ...
        float yStep = (float) height / (maxWeightValue - minWeightValue);

        // Draw data points
        float startX = 0;
        float startY = height - (weightData.get(0) - minWeightValue) * yStep;

        for (int i = 1; i < weightData.size(); i++) {
            float endX = startX + xStep;
            float endY = height - (weightData.get(i) - minWeightValue) * yStep;
    ...
    private void drawGrid(Canvas canvas, int width, int height, Paint paint) {
        ...
        for (int i = minWeightValue; i <= maxWeightValue; i++) { // Updated
            float y = height - (i - minWeightValue) * yStep; // Updated
